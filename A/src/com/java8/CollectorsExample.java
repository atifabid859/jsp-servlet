...
//List<Float> productPriceList =  productsList.stream() .map(x->x.price).collect(Collectors.toList());
        //Set<Float> productPriceList =  (Set<Float>) productsList.stream() .map(x->x.price).collect(Collectors.toSet());    
        Double sumPrice =productsList.stream().collect(Collectors.summingDouble(x->x.price));   
